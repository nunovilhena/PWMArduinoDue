...
static int PWM_MAX_DUTY = 1400; //max value duty cycle can take 9 bits
static int PWM_FREQ = 15000;

//*********************************************************************************************************************
//this will calculate the deadtime, however it is not used in this example, if your drive do not implement deadtime, you should used it by software and use next lines
//this is defined in setup_pwm

static int PWM_CLOCK = (PWM_FREQ * PWM_MAX_DUTY); //number of PWM ticks per second - autocalculated - don't modify this line
static int PWM_PICO_PER_TICK = (1000000000ul / (PWM_CLOCK / 1000ul)); //Number of picoSeconds per PWM clock tick - trust me, it's relevant
    //target dead time in nano seconds. So, 1000 is a micro second. You will need to tune this to the IGBT hardware.
static int PWM_TARGET_DEADTIME = 1400;
    //autocalculated from constant parameters. Don't change this, change the input parameters above
static int PWM_DEADTIME = (((PWM_TARGET_DEADTIME * 1000ul) / PWM_PICO_PER_TICK) + 1);
    //If the PWM pulse would be shorter than this many nano seconds then suppress it.
static int PWM_BUFFER_NANOS = 300;
    //this is an autocalculated value based on a bunch of the above constants. Do not change this equation.
static int PWM_BUFFER = (((PWM_BUFFER_NANOS * 500ul) / PWM_PICO_PER_TICK) + (PWM_DEADTIME * 2));
...
//*********************************************************************************************************************
//this will calculate the deadtime, however it is not used in this example
    PWM_CLOCK = (PWM_FREQ * PWM_MAX_DUTY); //number of PWM ticks per second - autocalculated - don't modify this line
    PWM_PICO_PER_TICK = (1000000000ul / (PWM_CLOCK / 1000ul)); //Number of picoSeconds per PWM clock tick - trust me, it's relevant
    //target dead time in nano seconds. So, 1000 is a micro second. You will need to tune this to the IGBT hardware.
    PWM_TARGET_DEADTIME = 1400;
    //autocalculated from constant parameters. Don't change this, change the input parameters above
    PWM_DEADTIME = (((PWM_TARGET_DEADTIME * 1000ul) / PWM_PICO_PER_TICK) + 1);
    //If the PWM pulse would be shorter than this many nano seconds then suppress it.
    PWM_BUFFER_NANOS = 300;
    //this is an autocalculated value based on a bunch of the above constants. Do not change this equation.
    PWM_BUFFER = (((PWM_BUFFER_NANOS * 500ul) / PWM_PICO_PER_TICK) + (PWM_DEADTIME * 2));
